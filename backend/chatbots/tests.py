...
from .signals import chat_post_save_signal
import pytest
from channels.testing import WebsocketCommunicator
from conversation_app.asgi import application
import time
import asyncio
...
class ChatAPITestCase(APITestCase):
    ...
    def setUp(self):        
        ...
        post_save.disconnect(chat_post_save_signal, sender=Chat) # Disconnect Chat signals before creating test chat objects
        for i in range(0, 3):
            Chat.objects.create(
                user=self.user_1, 
                theme=theme, 
        ...
        post_save.connect(chat_post_save_signal, sender=Chat) # Reconnect signals
    ...
    def test_create_chat(self):
        ...
        self.assertTrue(len(response.data["introduction"]) > 0)
...
class ChatMessagesAPIViewTestCase(APITestCase):
    ...
    def test_successful_chat_message(self):
        """
        Test a chatbot message generation with correct data
        """
        data = {
...
# class TestSpeechBot:
#     """
#     Test case for testing speech bot API endpoint using django websockets and channels
#     """

@pytest.mark.asyncio
async def test_audio_stream_websocket():
    # Test WebSocket API
    communicator = WebsocketCommunicator(application, "/ws/audio-stream/")
    connected, _ = await communicator.connect()
    assert connected
    print("CONNECTED ------------------------------------------")

    test_audio = "chatbots/voice_sample/test_voice.wav"
    with open(test_audio, "rb") as audio_file:
        audio_chunk = audio_file.read()

    try:
        # Send audio in a separate task
        send_task = asyncio.create_task(communicator.send_to(bytes_data=audio_chunk))

        # Start receive task in parallel
        receive_task = asyncio.create_task(communicator.receive_from(timeout=5))
        ...
        try:
            response = await asyncio.wait_for(receive_task, timeout=5)
            print("Response received:", response)
            assert  "transcription" in response
        except asyncio.TimeoutError:
            print("Test timed out waiting for WebSocket response.")
